...
public class Bpmn2JsonUnmarshaller {
    ...
    protected Bpmn2Resource unmarshall(JsonParser parser, String preProcessingData) throws JsonParseException, IOException {
        try {
            logTokens(parser); // open the object
...
//                } 
    ...
    private JsonToken logTokens(JsonParser parser) throws IOException {
        JsonToken token = parser.nextToken();
        System.out.println("Token=" + token.toString());
        return token;
    }
    
    private String getCurrentName(JsonParser parser) throws IOException {
        String name = parser.getCurrentName();
        System.out.println("getCurrentName=" + name);
        return name;
    }

    private String getText(JsonParser parser) throws IOException {
        String name = parser.getText();
        System.out.println("getText=" + name);
        return name;
    }

    private int getIntValue(JsonParser parser) throws IOException {
    ...
    private BaseElement unmarshallItem(JsonParser parser, String preProcessingData) throws JsonParseException, IOException {
        ...
        while (logTokens(parser) != JsonToken.END_OBJECT) {
            String fieldname = getCurrentName(parser);
            logTokens(parser);
            if ("resourceId".equals(fieldname)) {
                resourceId = getText(parser);
                ...
                logTokens(parser);
                logTokens(parser);
                stencil = getText(parser);
                logTokens(parser);
            } else if ("childShapes".equals(fieldname)) {
                while (logTokens(parser) != JsonToken.END_ARRAY) { // open the
                                                                    // object
                ...
                logTokens(parser);
                logTokens(parser);
                Integer x2 = getIntValue(parser);
                logTokens(parser);
                logTokens(parser);
                Integer y2 = getIntValue(parser);
                logTokens(parser);
                logTokens(parser);
                logTokens(parser);
                logTokens(parser);
                logTokens(parser);
                Integer x1 =getIntValue(parser);
                logTokens(parser);
                logTokens(parser);
                Integer y1 = getIntValue(parser);
                logTokens(parser);
                logTokens(parser);
            	...
            	JsonToken nextToken = logTokens(parser);
            	boolean end = JsonToken.END_ARRAY.equals(nextToken);
            	while (!end) {
            		nextToken = logTokens(parser);
            		nextToken = logTokens(parser);
            		Integer x = getIntValue(parser);
                    logTokens(parser);
                    logTokens(parser);
                    Integer y = getIntValue(parser);
                ...
                while (logTokens(parser) != JsonToken.END_ARRAY) {
                    // {resourceId: oryx_1AAA8C9A-39A5-42FC-8ED1-507A7F3728EA}
                    logTokens(parser);
                    logTokens(parser);
                    outgoing.add(getText(parser));
                    logTokens(parser);
                }
                // pass on the array
                parser.skipChildren();
                ...
                // }
                // add to the map:
                // logTokens(parser); // resourceId:
                // logTokens(parser); // the value we want to save
                // targetId = parser.getText();
                // logTokens(parser); // }, closing the object
            }
        }
...
//        }
    ...
    private Map<String, String> unmarshallProperties(JsonParser parser) throws JsonParseException, IOException {
        ...
        while (logTokens(parser) != JsonToken.END_OBJECT) {
            String fieldname = getCurrentName(parser);
            logTokens(parser);
            properties.put(fieldname, getText(parser));
        }
        return properties;
    }
